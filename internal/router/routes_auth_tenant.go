...
func RegisterSystemAdminRoutes(
	...
	adminRoutes := r.Group("/system/admin", g.SystemAdmin())
	{
		...
		if pluginHandler != nil {
			...
			plugins.GET("/marketplace", pluginHandler.Marketplace)
			plugins.GET("/type-specifications", pluginHandler.TypeSpecifications)
			plugins.POST("/inspect", pluginHandler.InspectManifest)
			plugins.POST("/operations/install", pluginHandler.StartInstallOperation)
			plugins.GET("/operations/:operation_id", pluginHandler.Operation)
			plugins.GET("/:id", pluginHandler.Get)
			plugins.PUT("/:id", pluginHandler.Upgrade)
			plugins.POST("/:id/operations/upgrade", pluginHandler.StartUpgradeOperation)
			plugins.POST("/:id/operations/upgrade-latest", pluginHandler.UpgradeLatest)
			plugins.POST("/:id/check-update", pluginHandler.CheckUpdate)
			plugins.GET("/:id/history", pluginHandler.History)
			...
			plugins.GET("/:id/events", pluginHandler.Events)
