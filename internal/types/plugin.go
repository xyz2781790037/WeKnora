...
type Plugin struct {
	...
	ConsecutiveHealthFailures int        `json:"consecutive_health_failures" gorm:"not null;default:0"`
	RecoveryAttempts          int        `json:"recovery_attempts" gorm:"not null;default:0"`
	LastRecoveryAt            *time.Time `json:"last_recovery_at"`
	SourceManifestURL         string     `json:"source_manifest_url" gorm:"type:text;not null;default:''"`
	LatestVersion             string     `json:"latest_version" gorm:"type:varchar(64);not null;default:''"`
	UpdateAvailable           bool       `json:"update_available" gorm:"not null;default:false"`
	UpdateCheckedAt           *time.Time `json:"update_checked_at"`
	UpdateMessage             string     `json:"update_message" gorm:"type:text;not null;default:''"`
